...
class ChatifyMessenger
{
    ...
    {
//        if ($user->user_picture == 'avatar.png' && config('chatify.gravatar.enabled')) {
//            $imageSize = config('chatify.gravatar.image_size');
//            $imageset = config('chatify.gravatar.imageset');
//            $user->user_picture = 'https://www.gravatar.com/avatar/' . md5(strtolower(trim($user->user_email))) . '?s=' . $imageSize . '&d=' . $imageset;
//        } else {
//            $user->user_picture = self::getUserAvatarUrl($user->user_picture);
//        }
        $user->user_picture = self::getUserAvatarUrl($user->user_picture);
        return $user;
    }
    ...
    {
//        $destination = Destination::where('id', $user_id)->first();
    ...
    public function makeInFavorite($user_id, $action)
    {
        if ($action > 0) {
            ...
            // Star for the other user
            $star = new Favorite();
            $star->id = rand(9, 99999999);
            $star->user_id = $user_id;
            $star->favorite_id = Auth::user()->id;
            $star->save();
            ...
            $star = Favorite::where('favorite_id', Auth::user()->id)->where('user_id', $user_id)->delete();
            return $star ? true : false;
        }
    }
...
//        return self::storage()->url(config('chatify.attachments.folder') . '/' . $attachment_name);
        return url(asset('storage/attachments/' . $attachment_name));
    }
}
