...
@section('content-tourist-show')
    ...
    <section id="show-header-banner">
        ...
        <div class="header show-animation"  style="">
            ...
            <p class="text-white">Manage Your Companies</p>
        </div>
    </section>
    @if(session('success'))
    ...
    @elseif(session('error'))
        <div class="toast bg-danger text-white fade" role="status" aria-live="polite"  aria-atomic="true" data-delay="8000" id="showToast" style="z-index: 5;position: absolute; top: 0; right: 0;">
            <div class="toast-header">
                <i class="fa-sharp fa-solid fa-circle rounded mr-2"></i>
                <strong class="mr-auto">Notification</strong>
                <small>seconds ago</small>
                ...
                <table id="touristRequests" class="table table-bordered table-light table-responsive-xl text-wrap" style="text-align: center">
                    ...
                    @foreach($pages as $request)
                        <tr>
                            ...
                            <td>
                                @if($request->operator_approval == 'Approved')
                                    <span class="badge bg-success" style="color: white !important;">{{ $request->operator_approval }}</span>
                                @elseif($request->operator_approval == 'Pending')
                                    <span class="badge bg-warning" style="color: white !important;">{{ $request->operator_approval }}</span>
                                @else
                                    <span class="badge bg-danger" style="color: white !important;">{{ $request->operator_approval }}</span>
                                @endif
                            </td>
                            <td>{{ Carbon\Carbon::parse($request->tour_created)->isoFormat('LLL') }}</td>
                            <td>{{ Carbon\Carbon::parse($request->tour_updated)->isoFormat('LLL') }}</td>
                            <td>
                                <form action="{{ route('tour_operator.show', $request->tour_id ) }}" method="GET">
