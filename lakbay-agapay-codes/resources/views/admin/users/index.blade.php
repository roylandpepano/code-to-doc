    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="ml-2">All Users</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6 header-btn">
                        <form action="{{ route('admin.users.tourist_user.index') }}" method="GET">
                            <button style="color: white !important;" type="submit" class="btn btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="View Tourist Users">
                                Tourists</button>
                        </form>
                        ...
                        @endif
                        <button type="button" class="btn btn-secondary" data-bs-toggle="tooltip" data-bs-placement="top" title="View All Users">
                            All Users</button>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
        ...
        <section class="content">
            <div class="container-fluid">
                <div class="card-body table-responsive table-tourist-requests table table-striped table-bordered table-light" style="height: 100vh;">
                    <table id="users" class="table table-striped table-bordered table-light table-responsive-xl text-nowrap">
                        <thead>
                        <tr>
                            <th>Type</th>
                            <th>Name</th>
                        ...
                        @foreach($users as $user)
                            @if((Auth::guard('web')->user()->user_type == 'Admin' && $user->user_type != 'Super Admin') ||
                                    (Auth::guard('web')->user()->user_type == 'Super Admin'))
                            <tr>
                                ...
                                <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                                    @csrf
                                    <button class="btn btn-outline-primary btn-sm" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="View Record"
                                            onclick="location.href='{{ route('admin.users.show', $user->id) }}'">
                                        <i class="fa-solid fa-eye mr-1"></i> Details</button>
                                    @if($user->user_type != 'Admin' && $user->user_type != 'Super Admin' && Auth::guard('web')->user()->user_type == 'Super Admin' && $user->user_type != 'Tour Operator')
                                    ...
                                    @endif
                                    @if($user->user_type == 'Admin' && Auth::guard('web')->user()->user_type == 'Super Admin')
                                        <button class="btn btn-outline-danger btn-sm" data-bs-toggle="tooltip" name="submit" type="submit" value="remove" data-bs-placement="top" title="Remove Admin">
                                            <i class="fa-solid fa-user-minus mr-1"></i>Remove Admin</button>
                                    @endif
                                </form>
                                </td>
                            </tr>
